...
export const PlaygroundSection: React.FC<PlaygroundSectionProps> = ({ initialPrompt, onPromptUsed }) => {
  ...
  return (
    <div className="space-y-10 animate-fade-in max-w-5xl mx-auto" ref={topRef}>
      ...
      <Card className={`bg-gray-850 transition-all duration-300 ${isChaining ? 'ring-2 ring-blue-500 ring-opacity-50 scale-[1.01]' : ''}`}>
        <div className="space-y-6">
          <div className="relative">
            <label htmlFor="userInput" className="block text-sm font-medium text-blue-300 mb-2">
          ...
          <div>
            <label htmlFor="customInstructions" className="block text-sm font-medium text-blue-300 mb-2">
            ...
            />
             <p className="mt-2 text-xs text-gray-400">
              Examples: desired tone, style, length, target audience, key elements to include/exclude.
            </p>
          </div>
          ...
          {error && (
            <div role="alert" className="p-4 bg-red-500/10 border border-red-500/30 rounded-xl text-red-300 text-sm">
      ...
      {generatedPrompt && (
        <Card title="✨ Genius-Level Prompt ✨" className="bg-gray-850 animate-slide-up">
          <div className="relative group">
            ...
            <pre className="relative whitespace-pre-wrap p-6 bg-gray-900 rounded-xl text-gray-200 text-sm overflow-x-auto border border-gray-700 shadow-2xl font-mono leading-relaxed" aria-label="Generated enhanced prompt">
            ...
            <div className="absolute top-4 right-4 flex gap-2">
              ...
              >
                ...
                <span className="hidden sm:inline text-[10px] font-bold tracking-wider uppercase">Chain</span>
              </button>
              <button
              ...
              >
                {copied ? <CheckIcon className="w-4 h-4 text-teal-400" /> : <ClipboardIcon className="w-4 h-4" aria-hidden="true" />}
              </button>
            </div>
          </div>
          {shareStatus && <p className="text-xs text-teal-400 mt-2 text-right">{shareStatus}</p>}
                    ...
                    <div key={item.id} className="p-4 bg-gray-900/50 rounded-xl border border-gray-700/50 hover:border-gray-600 transition-colors">
                        <div className="flex justify-between items-start">
                            <div className="flex-1 min-w-0">
                                <p className="text-sm font-medium text-blue-300 truncate" title={item.userInput}>
                                    {item.userInput}
                                </p>
                            ...
                            <div className="flex gap-2 flex-shrink-0 ml-4">
                                ...
                                <button onClick={() => handleReuseHistory(item)} className="px-3 py-1 text-xs font-semibold bg-blue-600/20 text-blue-300 border border-blue-500/30 rounded-lg hover:bg-blue-600 hover:text-white transition-all">
                                    Reuse
                                </button>
                            </div>
                        </div>
                        <details className="mt-3 group">
                            ...
                            <div className="mt-3 p-3 bg-gray-950 rounded-lg border border-gray-800">
                                {item.customInstructions && (
                                  <div className="mb-3">
                                    <p className="text-[10px] uppercase font-bold text-gray-600 mb-1">Instructions</p>
                                    <p className="text-xs text-gray-400">{item.customInstructions}</p>
                                  </div>
                                )}
                                <div>
                                    <p className="text-[10px] uppercase font-bold text-gray-600 mb-1">Result</p>
                                    <pre className="text-xs text-gray-300 whitespace-pre-wrap font-mono"><code>{item.generatedPrompt}</code></pre>
                                </div>
                            </div>
                        </details>
                    </div>
