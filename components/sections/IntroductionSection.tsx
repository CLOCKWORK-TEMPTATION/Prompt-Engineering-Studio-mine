...
const renderContentNode = (node: string | React.ReactNode): React.ReactNode => {
  if (typeof node === 'string') {
    return node.split('\n').map((line, index) => <p key={index} className="leading-7">{line}</p>);
  }
  return node;
};
  ...
  return (
    <div className="space-y-10 animate-fade-in max-w-5xl mx-auto">
      ...
      <Card title={understandingPromptsContent.title} className="bg-gray-850">
        {understandingPromptsContent.description && <div className="text-gray-300 mb-6 space-y-3">{renderContentNode(understandingPromptsContent.description)}</div>}
        <div className="space-y-4">
          {understandingPromptsContent.subItems?.map((item: ContentBlock) => (
            <AccordionItem key={item.id} title={item.title}>
              {item.details && <div className="text-sm text-gray-400 space-y-3 leading-relaxed">{renderContentNode(item.details)}</div>}
            </AccordionItem>
          ))}
        </div>
      </Card>
