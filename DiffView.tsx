...
import { ClipboardIcon, CheckIcon, ChevronDownIcon, ChevronUpIcon, EyeIcon, CodeBracketIcon } from '../Icons';
...
export const DiffView: React.FC<DiffViewProps> = ({
  ...
  const calculateDiff = (): DiffLine[] => {
    ...
    while (i < originalLines.length || j < optimizedLines.length) {
      if (i >= originalLines.length) {
        diff.push({ content: optimizedLines[j], type: 'added', lineNumber: j + 1 });
        j++;
      } else if (j >= optimizedLines.length) {
        diff.push({ content: originalLines[i], type: 'removed', lineNumber: i + 1 });
        i++;
      } else if (originalLines[i] === optimizedLines[j]) {
        diff.push({ content: originalLines[i], type: 'unchanged', lineNumber: i + 1 });
        i++;
        j++;
      } else {
  ...
  const unchangedPercentage = Math.round((stats.unchanged / stats.total) * 100);
      ...
      <div className="p-4 border-b border-gray-700/50">
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-lg font-semibold text-gray-200">Before & After</h3>
          </div>
      ...
      <div className="overflow-auto">
        {viewMode === 'side-by-side' && (
          <div className="grid grid-cols-2 divide-x divide-gray-700/50">
            ...
            <div className="p-4">
              <div className="flex items-center justify-between mb-3">
                <span className="text-xs font-semibold text-red-300 uppercase tracking-wide">Original</span>
                <span className="text-xs text-gray-500">{original.split('\n').length} lines</span>
              </div>
            ...
            <div className="p-4">
              <div className="flex items-center justify-between mb-3">
                <span className="text-xs font-semibold text-emerald-300 uppercase tracking-wide">Optimized</span>
                <span className="text-xs text-gray-500">{optimized.split('\n').length} lines</span>
              </div>
                    ...
                    {diff.map((line, index) => (
                      ...
                      >
                        ...
                        <td
                          ...
                          dangerouslySetInnerHTML={{ __html: getLineIcon(line.type) + ' ' + line.content }}
                        />
                      </tr>
                    ))}
                  </tbody>
...
export const InlineComparison: React.FC<InlineComparisonProps> = ({
  ...
  return (
    <div className={`bg-gray-850 backdrop-blur-xl border border-gray-700 rounded-2xl overflow-hidden ${className}`}>
      ...
      <div className="p-4">
        <div className={`bg-gray-900 rounded-xl p-4 shadow-inner transition-all ${showAfter ? 'border border-emerald-500/20' : ''}`}>
          <pre className={`text-sm font-mono leading-relaxed whitespace-pre-wrap ${showAfter ? 'text-gray-200' : 'text-gray-400'}`}>
...
export const DiffIndicator: React.FC<DiffIndicatorProps> = ({ diffCount, className = '' }) => {
  return (
    <div className={`flex items-center gap-2 ${className}`}>
      <div className="w-2 h-2 rounded-full bg-emerald-400 animate-pulse" />
      <span className="text-xs text-gray-400">{diffCount} change{diffCount !== 1 ? 's' : ''}</span>
    </div>
  );
};
