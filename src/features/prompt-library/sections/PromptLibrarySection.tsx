...
const TemplateCard: React.FC<{
  ...
  return (
    <Card
      ...
      className="bg-gray-850 flex flex-col h-full hover:shadow-glow-primary transition-shadow duration-300"
    >
      <div className="flex-grow">
        <p className="text-xs text-blue-300/80 uppercase tracking-wide font-bold mb-2">
          {template.useCase}
        </p>
          ...
          <ul className="space-y-1.5 text-xs text-gray-400">
            {template.variables.map((v) => (
              <li key={v.name} className="flex gap-2">
                <code className="bg-gray-800 px-1.5 py-0.5 rounded text-blue-200 font-mono text-[10px] border border-gray-700">
                  {v.name}
                </code>
