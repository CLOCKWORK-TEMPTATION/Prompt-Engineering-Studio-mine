    ...
    >
        ...
        {variants.map((variant) => {
          ...
          return (
            ...
            >
              {isActive && (
                <div
                  className={`absolute bottom-0 left-0 right-0 h-0.5 ${config.borderColor.replace('/30', '')}`}
                />
              )}
      ...
      {activeVariant && (
        <div className="p-6">
          ...
          <div
            className={`mt-4 p-4 rounded-xl border ${VARIANT_CONFIG[activeVariant.platform].bgColor} ${VARIANT_CONFIG[activeVariant.platform].borderColor} border`}
          >
            <p
...
export interface CompactVariantTabsProps extends Omit<
  VariantTabsProps,
  'className'
> {
  className?: string;
