    ...
    return node.split('\n').map((line, index) => {
      ...
      if (line.startsWith('```') && line.length > 3) {
        return (
          <p
            key={index}
            className="font-mono bg-gray-800 border border-gray-700 p-2 rounded-md text-sm text-blue-300 my-2"
          >
            {line.substring(3)}
          </p>
        );
      ...
      return (
        <p
          ...
          className={
            isCodeLine
              ? 'font-mono bg-gray-700/50 px-1.5 py-0.5 rounded text-sm text-blue-200 inline-block'
              : 'leading-7'
          }
        >
          {content}
      ...
      <header className="pb-8 border-b border-gray-700/50">
        ...
        {introduction && (
          <div className="mt-4 text-lg text-gray-400 space-y-2 leading-relaxed max-w-3xl">
            {renderContentNode(introduction)}
          </div>
        )}
      </header>
        ...
        {contentBlocks.map((block) =>
          layout === 'accordion' ? (
            <AccordionItem
              key={block.id}
              title={block.title}
              initiallyOpen={block.id === contentBlocks[0].id}
            >
              {block.description && (
              ...
              {block.examplePrompt && (
                <div className="mt-4">
                  ...
                  <pre className="bg-gray-900 border border-gray-700 p-4 rounded-xl text-sm text-gray-300 overflow-x-auto whitespace-pre-wrap shadow-inner font-mono">
                    <code>
                      {typeof block.examplePrompt === 'string'
                        ? block.examplePrompt
                        : renderContentNode(block.examplePrompt)}
                    </code>
                  </pre>
                </div>
              )}
                    ...
                    <div key={subItem.id} className="group">
                      ...
                      {subItem.examplePrompt && (
                        <pre className="bg-gray-800/50 p-3 rounded-lg text-xs text-gray-300 overflow-x-auto mt-2 whitespace-pre-wrap border border-gray-700/50">
                          <code>
                            {typeof subItem.examplePrompt === 'string'
                              ? subItem.examplePrompt
                              : renderContentNode(subItem.examplePrompt)}
                          </code>
                        </pre>
                      )}
                    </div>
                ...
                <div className="mt-6 space-y-4">
                  {block.subItems.map((subItem) => (
                    <AccordionItem key={subItem.id} title={subItem.title}>
                      {subItem.details && (
                        <div className="text-sm text-gray-400 mt-2 mb-3 space-y-1">
                          {renderContentNode(subItem.details)}
                        </div>
                      )}
                      {subItem.examplePrompt && (
