...
export const optimizeUserPromptStructured = async (
  ...
  try {
    ...
    const validationResult = parseOptimizerResponse(parsedData);
    ...
    throw new Error(
      `AI response validation failed: ${(validationResult as { success: false; error: string }).error}\n\nParsed data:\n${JSON.stringify(parsedData, null, 2)}`
    );
...
Please format your response as a structured Markdown list. For each suggested User Prompt, provide a brief rationale in parentheses explaining *why* this is a great example usage.`;
  ...
  try {
    const response: GenerateContentResponse = await ai.models.generateContent({
      ...
      config: {
        systemInstruction: mode === 'test' ? qaSystemInstruction : exampleSystemInstruction,
        temperature: 0.7,
      }
    });
